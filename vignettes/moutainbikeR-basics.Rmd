...
some_activities_3 <- my_mbr$get_activities(
  activities_per_page = 200
  , page_number = 3
  #, after_date = "2018-01-01"
    )
some_activities_4 <- my_mbr$get_activities(
  activities_per_page = 200
  , page_number = 4
  #, after_date = "2018-01-01"
    )
...
```{r}
some_activities$year <- lubridate::year(some_activities$start_date)
library(ggplot2)
...
```{r}
activity_id <- "6050578290"

my_activity <- my_mbr$get_activity(
...
```{r}
my_activity_stream_lag_long <- my_mbr$get_activity_streams(activity_id = activity_id, streams_types = c("latlng"))
```
